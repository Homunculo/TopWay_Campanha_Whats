...
class WhatsApp:
    ...
    def __init__(self, wait, screenshot=None, session=None):
        ...
        if session:
            ...
            self.browser = webdriver.Chrome('WhatsAppBot/chromedriver.exe',
                                            options=chrome_options)  # we are using chrome as our webbrowser
        else:
            self.browser = webdriver.Chrome('WhatsAppBot/chromedriver.exe')
        self.browser.get("https://web.whatsapp.com/")
        # emoji.json is a json file which contains all the emojis
        with open("WhatsAppBot/emoji.json") as emojies:
    ...
    def send_picture(self, name, picture_location, caption=None):
        ...
        search.send_keys(name + Keys.ENTER)  # we will send the name to the input key box
        try:
            attach_xpath = '/html/body/div[1]/div/div/div[4]/div/footer/div[1]/div[1]/div[2]/div/div'
